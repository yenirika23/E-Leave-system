...
@section('breadcrumb', 'HRD › Laporan › Detail Pengajuan')
...
<div class="row g-4">
    ...
    <div class="col-lg-7">
            ...
            <div class="card-body p-0">
                ...
                @php
                    $rows = [
                        ...
                        ['icon'=>'bi-clock-fill',           'label'=>'Total Hari',     'value'=>$leaveRequest->getTotalLabel()],
                ...
                @if($leaveRequest->document_path)
                    <div style="display:flex;align-items:flex-start;gap:14px;padding:13px 20px;border-bottom:1px solid #f1f5f9;">
                        ...
                        <div style="flex:1;">
                            <div style="font-size:11px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.4px;">Dokumen Lampiran</div>
                            <div style="font-size:13.5px;color:#1e293b;font-weight:600;margin-top:2px;">
                                {{ $leaveRequest->document_name }}
                                @if($leaveRequest->isApproved())
                                    <a href="{{ asset('storage/' . $leaveRequest->document_path) }}" target="_blank"
                                       style="color:#1a56db;text-decoration:none;margin-left:8px;font-size:12px;">
                                        <i class="bi bi-download"></i> Download
                                    </a>
                                @else
                                    <span style="color:#94a3b8;font-size:12px;margin-left:8px;">(Download tersedia setelah disetujui)</span>
                                @endif
                            </div>
                        </div>
                    </div>
        ...
        @if(Auth::user()->isHrd() && $leaveRequest->isPending() && $leaveRequest->user->supervisor?->isOnCuti())
            <div class="mt-3 d-flex flex-wrap gap-2">
                <form method="POST" action="{{ route('hrd.leave-approval.process', $leaveRequest) }}">
                    @csrf
                    <input type="hidden" name="action" value="disetujui">
                    <button type="submit" class="btn-green-soft" style="padding:10px 16px;">
                        <i class="bi bi-check-circle-fill me-2"></i> Approve
                    </button>
                </form>
                <button type="button" class="btn-red-soft" style="padding:10px 16px;" onclick="promptReject()">
                    <i class="bi bi-x-circle-fill me-2"></i> Reject
                </button>
            </div>
            <form id="rejectForm" method="POST" action="{{ route('hrd.leave-approval.process', $leaveRequest) }}" style="display:none;">
                @csrf
                ...
                <div class="timeline">
                    ...
                    <div class="timeline-item">
                        ...
                        <div style="margin-top:6px;background:#f0f4ff;border-radius:8px;padding:8px 12px;font-size:12px;color:#475569;">
                            {{ $leaveRequest->leaveType->name }} · {{ $leaveRequest->getTotalLabel() }}
                        </div>
                    </div>
